...
- For the slash controller, a maintainer's `/astra <task>`, `/sol <task>` or `/terra <task>` explicitly authorizes the documented issue response or issue-to-PR lifecycle through readiness, including review replies and thread resolution. It does not authorize merging or requesting reviewers.
