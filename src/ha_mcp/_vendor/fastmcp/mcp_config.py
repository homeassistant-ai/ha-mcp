...
if TYPE_CHECKING:
    from ha_mcp._vendor.fastmcp.client.transports import (
        ClientTransport,
        FastMCPTransport,
        SSETransport,
        StdioTransport,
...
class _TransformingMCPServerMixin(BaseModel):
    ...
    def _to_server_and_underlying_transport(
        ...
        try:
            ...
            from ha_mcp._vendor.fastmcp.server import create_proxy
    ...
    def to_transport(self) -> FastMCPTransport:
        ...
        try:
            from ha_mcp._vendor.fastmcp.client.transports import FastMCPTransport
        except ImportError as exc:
            raise ImportError(
                _install_hints.full_package(
                    "MCP configs that use FastMCP-specific tool transforms or tag filters"
