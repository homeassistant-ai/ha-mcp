...
from ha_mcp._vendor.fastmcp.settings import Settings
from ha_mcp._vendor.fastmcp.utilities.logging import configure_logging as _configure_logging

if TYPE_CHECKING:
    from ha_mcp._vendor.fastmcp.client import Client as Client
    from ha_mcp._vendor.fastmcp.client.group import ClientGroup as ClientGroup
    from ha_mcp._vendor.fastmcp.apps.app import FastMCPApp as FastMCPApp
    from ha_mcp._vendor.fastmcp.server.context import Context as Context
    from ha_mcp._vendor.fastmcp.server.server import FastMCP as FastMCP
...
from ha_mcp._vendor.fastmcp import _compat
...
def __getattr__(name: str) -> object:
    ...
    if name == "ClientGroup":
        try:
            from ha_mcp._vendor.fastmcp.client.group import ClientGroup
        except ImportError as exc:
            raise ImportError(_install_hints.CLIENT_SUPPORT) from exc
    ...
    if name == "client":
        try:
            return importlib.import_module("ha_mcp._vendor.fastmcp.client")
        except ImportError as exc:
            raise ImportError(_install_hints.CLIENT_SUPPORT) from exc
    if name == "server":
        try:
            return importlib.import_module("ha_mcp._vendor.fastmcp.server")
        except ImportError as exc:
            raise ImportError(_install_hints.SERVER_SUPPORT) from exc
