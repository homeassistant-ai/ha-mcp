...
const ADVANCED_FIELD_META = {
  ...
  dev_tools_security_policy_access: { label: "Dev tools security policy access", help: "⚠ DANGER: lets the developer tools rewrite tool security policies, add or remove per-tool approval gates, and approve or deny pending approvals on your behalf — an AI agent can accept its own gated calls. For policy testing only. Takes effect without a restart." },
};
...
function renderAdvancedSection(containerId, fields) {
  ...
  el.querySelectorAll('[data-adv-field]').forEach(input => {
    input.addEventListener('change', () => {
      ...
      // is armed independently of the dev-mode toggle above.
      if (fname === 'dev_tools_security_policy_access' && input.checked && !confirm(
        t(
          'advanced.dev_tools_security_policy_access.confirm',
          {},
          '⚠ Let developer tools change security policies?\n\nConnected AI agents will be able to rewrite tool security policies, add or remove per-tool approval gates, and approve or deny pending approvals — including their own gated calls. Enable only while testing policies.'
        )
      )) {
        input.checked = false;
        return;
