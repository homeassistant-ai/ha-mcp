...
function renderFsCustomPathsSubForm(parentEl, masterOn, fsOn) {
  ...
  info.innerHTML =
    ...
    `<div class="feature-help">${tHtml('filesystem.custom.help', {}, 'Extra paths (one per line) that file tools may <strong>read and write</strong>. Relative config paths and allowed HAOS sibling volumes are supported. Each entry allows that path and anything below it; config-relative filenames, such as <code>sensor.yaml</code>, can be entered directly. You can also manage these paths in Home Assistant under Settings → Devices & Services → HA-MCP Custom Component → HA-MCP File & YAML Tools → Configure; both locations edit the same setting and apply changes immediately.')}</div>` +
