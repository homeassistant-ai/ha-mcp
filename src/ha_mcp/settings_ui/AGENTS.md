...
- `tlh` is the only best-effort locale: it is excluded from machine translation; catalog, registration, generated-drift, and generated add-on schema-coverage problems warn. Settings-UI completeness and literal parity are intentionally not checked. Invalid Klingon is skipped or falls back to English; every other locale remains strict. Keep this exception centralized in `_locale_policy.py`.
