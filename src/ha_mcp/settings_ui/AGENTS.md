...
- `_i18n.py` and `locales/*.json` — auto-discovered translation catalogs. [`locales/README.md`](locales/README.md) owns the four-surface procedure, fallback rules, generated projections, CI gates, and the best-effort `tlh` exception.
- Packaging declarations include locale files by pattern; keep those patterns intact, but a new catalog needs no separate packaging registration.
