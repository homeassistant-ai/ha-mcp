...
def _supervisor_version_key(version: object) -> tuple[int, int, int]:
    ...
    try:
        ...
        raise RuntimeError(
            f"Supervisor returned an invalid version: {version!r}"
        ) from exc
    padded = numbers + (0, 0, 0)
    return padded[0], padded[1], padded[2]


def _post_supervisor_ws(
    ws: Any,
    next_id: Callable[[], int],
    endpoint: str,
    op_timeout: float,
    ...
    """Post one Supervisor command through Home Assistant's WS proxy."""
    msg_id = next_id()
    ...
    _await_supervisor_ws_result(
        ws,
        msg_id,
        endpoint,
        min(deadline, time.monotonic() + op_timeout),
    )


def _wait_supervisor_info(
    ws: Any,
    deadline: float,
    next_id: Callable[[], int],
) -> dict[str, Any]:
    """Read Supervisor info, tolerating transient reload failures."""
    last_error: str | None = None
    while time.monotonic() < deadline:
        msg_id = next_id()
        ws.send(
        ...
        result, error = _recv_supervisor_info_frame(ws, msg_id, deadline)
        if error is None and result is not None:
            return result
        if error is not None:
            last_error = error
            LOG.debug("Transient /supervisor/info failure: %s", error)
        time.sleep(5.0)
    suffix = f"; last error: {last_error}" if last_error else ""
    raise TimeoutError(
        "Supervisor info did not become available after channel reload" + suffix
    )


def _supervisor_channel_metadata_ready(
    info: dict[str, Any],
    channel: str,
    minimum_key: tuple[int, int, int] | None,
) -> bool:
    """Return whether channel metadata exposes a usable target version."""
    if info.get("channel") != channel:
        return False
    if minimum_key is None:
        return True
    if _supervisor_version_key(info.get("version")) >= minimum_key:
        return True
    latest_version = info.get("version_latest")
    return (
        bool(info.get("update_available"))
        and isinstance(latest_version, str)
        and _supervisor_version_key(latest_version) >= minimum_key
    )
...
def _wait_supervisor_channel_metadata(
    ...
    """Wait for a channel reload to expose the requested release."""
    last_info: dict[str, Any] | None = None
    while time.monotonic() < deadline:
        last_info = _wait_supervisor_info(ws, deadline, next_id)
        if _supervisor_channel_metadata_ready(last_info, channel, minimum_key):
            return last_info
        time.sleep(5.0)
    raise TimeoutError(
        "Supervisor channel metadata did not refresh before the deadline: "
        f"requested={channel!r}, minimum={minimum_version!r}, "
        f"reported={last_info!r}"
    )


def _validate_supervisor_channel(
    info: dict[str, Any],
    channel: str,
    minimum_version: str | None,
    minimum_key: tuple[int, int, int] | None,
) -> None:
    """Validate the settled Supervisor channel and minimum version."""
    if info.get("channel") != channel:
        raise RuntimeError(
            f"Supervisor channel is {info.get('channel')!r}, expected {channel!r}"
        )
    if (
        minimum_key is not None
        and _supervisor_version_key(info.get("version")) < minimum_key
    ):
        raise RuntimeError(
            f"Supervisor {info.get('version')!r} is below required "
            f"{minimum_version!r} on channel {channel!r}"
        )


def ensure_supervisor_channel(
    base_url: str,
    token: str,
    *,
    channel: str,
    ...
    """Move the HAOS test VM to a Supervisor channel and verify its version.

    This runs from the external test harness with its HA administrator token,
    before the PR add-on starts. It intentionally uses Core's WebSocket proxy:
    Supervisor 2026.08 only blocks that proxy for commands originating inside
    an add-on, which is the behavior the subsequently started add-on exercises.
    """
    import websockets.sync.client
    ...
    LOG.info(
        "Ensuring Supervisor channel=%s minimum_version=%s",
        channel,
    ...
    with websockets.sync.client.connect(ws_url, max_size=None, open_timeout=30) as ws:
        ...
        _post_supervisor_ws(ws, _next_id, "/supervisor/reload", 120.0, deadline)
        info = _wait_supervisor_channel_metadata(
            ws,
            deadline,
            _next_id,
            channel,
        ...
        if info.get("update_available"):
            _post_supervisor_ws(ws, _next_id, "/supervisor/update", timeout, deadline)
            info = _wait_supervisor_update_done(ws, deadline, _next_id)

    _validate_supervisor_channel(info, channel, minimum_version, minimum_key)
    LOG.info(
        "Supervisor channel ready: channel=%s version=%s",
        info.get("channel"),
        info.get("version"),
    )
    return info
