...
The helper is the shared assembly point for the attach_skill_payload parameter on
...
# ---------------------------------------------------------------------------
# attach_skill_payload=True path: canonical files attached
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# attach_skill_payload=False path: canonical files suppressed
# ---------------------------------------------------------------------------
...
class TestIncludeSkillOff:
    ...
    def test_referenced_files_still_attach_when_off(self, patched_get_skills_dir):
        """attach_skill_payload=False suppresses canonical defaults but BP-warning
...
class TestDegradedPaths:
    ...
    def test_nothing_requested(self, patched_get_skills_dir):
        """attach_skill_payload=False with no referenced_files returns empty without I/O."""
        result = build_skill_content(
            attach_skill_payload=False,
            canonical_files=("references/automation-patterns.md",),
            referenced_files=None,
...
class TestAttachSkillContent:
    ...
    def test_nothing_requested_is_silent(self, patched_get_skills_dir):
        """attach_skill_payload=False + no referenced_files → silent (user opted out)."""
        response: dict = {"success": True}
        attach_skill_content(
            response,
            attach_skill_payload=False,
    ...
    def test_vendor_missing_with_attach_skill_payload_warns(self):
    ...
    def test_vendor_missing_with_opt_out_is_silent(self):
        """Caller explicitly opted out with attach_skill_payload=False — don't nag."""
        with patch("ha_mcp.utils.skill_loader.get_skills_dir", return_value=None):
            response: dict = {"success": True}
            attach_skill_content(
                response,
