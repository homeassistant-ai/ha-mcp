...
async def test_native_edit_cannot_bypass_dashboard_tool(command):
    ...
    with pytest.raises(
        ToolError, match="Use the dedicated ha-mcp tools instead"
    ):
...
async def test_native_edit_cannot_bypass_dashboard_tool_in_sandbox():
    ...
    bridge = _SandboxBridge(MagicMock(), client, SimpleNamespace(code_mode_max_invocations=10))
    result = await bridge.ws_send({"type": "ha_mcp_tools/dashboard_edit", "config": {"views": []}})
