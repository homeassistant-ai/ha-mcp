...
class TestLabelLifecycle:
    ...
    async def _cleanup_test_labels(self, mcp_client, label_ids: list[str]) -> None:
        ...
        for label_id in label_ids:
            try:
                await proxy_call_tool(mcp_client,
    ...
    async def test_basic_label_lifecycle(self, mcp_client, cleanup_tracker):
        ...
        try:
            ...
            list_result = await proxy_call_tool(mcp_client, "ha_config_get_label", {})
            ...
            update_result = await proxy_call_tool(mcp_client,
                "ha_config_set_label",
                {
                    "label_id": label_id,
                    "name": new_name,
            ...
            delete_result = await proxy_call_tool(mcp_client,
                "ha_config_remove_label",
                {"label_id": label_id},
            )
