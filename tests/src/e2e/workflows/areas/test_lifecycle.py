...
- Floor creation, listing, updating, and deletion (via type='floor' parameter)
...
class TestAreaLifecycle:
    ...
    async def test_area_create_list_delete(self, mcp_client, cleanup_tracker):
        ...
        list_result = await mcp_client.call_tool("ha_config_list_areas_or_floors", {})
        ...
        verify_result = await mcp_client.call_tool("ha_config_list_areas_or_floors", {})
...
class TestFloorLifecycle:
    """Test complete floor management workflows via ha_config_*_area(type='floor')."""

    async def test_floor_create_list_delete(self, mcp_client, cleanup_tracker):
        """
        Test: Create floor -> List floors -> Delete floor

        Validates basic floor CRUD operations using consolidated area tools with type='floor'.
        """
        ...
        list_result = await mcp_client.call_tool(
            "ha_config_list_areas_or_floors", {"type": "floor"}
        )
...
class TestAreaFloorIntegration:
    ...
    async def test_area_with_floor_assignment(self, mcp_client, cleanup_tracker):
        ...
        await mcp_client.call_tool(
            "ha_config_remove_area_or_floor", {"type": "floor", "floor_id": floor_id}
        )
    ...
    async def test_multiple_areas_on_floor(self, mcp_client, cleanup_tracker):
        ...
        for area_id in area_ids:
            delete_result = await mcp_client.call_tool(
                "ha_config_remove_area_or_floor", {"area_id": area_id}
            )
            delete_data = parse_mcp_result(delete_result)
...
async def test_floor_list_empty_or_populated(mcp_client):
    ...
    list_result = await mcp_client.call_tool("ha_config_list_areas_or_floors", {"type": "floor"})
    list_data = parse_mcp_result(list_result)
