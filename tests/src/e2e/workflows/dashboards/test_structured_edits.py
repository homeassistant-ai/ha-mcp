...
async def test_reporter_dashboard_edits_and_conflicts(
    ...
    try:
        ...
        await safe_call_tool(mcp_client, "ha_config_delete_dashboard", {"url_path": path})


@pytest.mark.asyncio
async def test_native_dashboard_backend_measurements(mcp_client, ha_client, record_property):
    ...
    if "dashboard_edit" not in (info.get("result") or {}).get("capabilities", []):
        ...
        record_property("dashboard_backend_measurement", "component absent; public fallback tested separately")
        return
    ...
    try:
        ...
        current = await mcp.call_tool_success("ha_config_get_dashboard", {"url_path": path})
        current_hash = current["config_hash"]
        # Alternate order to reduce warm-up/order bias; all edits are actual changes.
        for iteration in range(8):
            for mode in (["legacy", "native"] if iteration % 2 == 0 else ["native", "legacy"]):
                ...
                started = time.monotonic()
                if mode == "legacy":
                    document, received = await exchange({"type": "lovelace/config", "url_path": path, "force": True})
                    assert compute_config_hash(document) == current_hash
                    document["views"][0]["sections"][1]["cards"][0]["icon"] = icon
                    _, saved = await exchange({"type": "lovelace/config/save", "url_path": path, "config": document})
                    verified, reloaded = await exchange({"type": "lovelace/config", "url_path": path, "force": True})
                    byte_count, calls = received + saved + reloaded, 3
                    current_hash = compute_config_hash(verified)
                else:
                    result, byte_count = await exchange({"type": "ha_mcp_tools/dashboard_edit", "url_path": path, "expected_hash": current_hash, "patch": [{"op": "replace", "path": "/views/0/sections/1/cards/0/icon", "value": icon}]})
                ...
                measurements[mode].append({"seconds": time.monotonic() - started, "bytes": byte_count, "commands": calls})
        assert max(row["bytes"] for row in measurements["native"]) < min(row["bytes"] for row in measurements["legacy"])
