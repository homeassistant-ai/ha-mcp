#!/usr/bin/env python3
"""Home Assistant MCP Server Add-on startup script (Dev Channel).

Extends the base startup with webhook proxy support for remote access
via Nabu Casa, Cloudflare, DuckDNS, nginx, or any reverse proxy.
"""
...
# ---------------------------------------------------------------------------
# Dual-port server (main port + ingress port)
# ---------------------------------------------------------------------------
...
def main() -> int:
    ...
    log_info("Starting Home Assistant MCP Server (Dev)...")
