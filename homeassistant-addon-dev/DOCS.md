...
| `read_only_mode` | Toggles all write tools off, and removes ability for tools to make any write or destructive calls. Mixed read/write tools (backups, add-ons, energy preferences, voice pipelines) stay available with their write operations blocked. Same toggle as the web UI Tools tab. | `false` |
