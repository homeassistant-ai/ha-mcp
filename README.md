...
Template helper edit backups capture persisted options through the HA-MCP custom component, using either its Server entry or File & YAML Tools entry. Capture refuses degraded secret scrubbing and uses the stable config-entry identity, including for older alias-backed history. Restore requires the original entry and a fresh safety backup, removes optional settings absent from the snapshot, and verifies the persisted result. Helper and integration edits through the same server wait until that restore finishes. An unavailable or mismatched verification is reported as a failure that may have applied the change; inspect the current options before retrying. Entity/device registry metadata is outside this snapshot, and deleted entries are not recreated.
