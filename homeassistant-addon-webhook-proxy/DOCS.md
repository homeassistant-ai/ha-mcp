...
All three behaviors advertise proxy-owned endpoints under `/api/mcp_proxy/oauth` (plus the webhook-scoped `.well-known` document), so a cached client configuration remains on the proxy across mode switches:
